...
import { LogOut, User, Phone, Bus as BusIcon, Route, ShieldCheck, BadgeCheck, AlertTriangle } from 'lucide-react';
      ...
      <div className="w-full max-w-2xl mx-auto bg-white rounded-t-3xl lg:rounded-3xl lg:shadow-xl lg:border lg:border-slate-150 overflow-hidden">
        ...
        <div className="p-8 pt-12 bg-white border-b border-slate-100 flex flex-col items-center shrink-0 shadow-sm">
          ...
          <div className="flex gap-2 mt-6">
            <button 
              onClick={onLogout}
              className="flex items-center gap-2 px-6 py-2.5 bg-slate-50 hover:bg-slate-100 text-slate-400 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95"
            >
            ...
            {onEmergency && (
              <button 
                onClick={onEmergency}
                className="flex items-center gap-2 px-6 py-2.5 bg-red-50 hover:bg-red-100 text-red-600 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95"
              >
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
              ...
              {route && onToggleDirection && (
                <>
                  <div className="h-px bg-slate-50 w-full" />
                  <div className="space-y-3">
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Active Trip Shift</span>
                    <div className="flex gap-2 bg-slate-100 p-1 rounded-2xl">
                      <button
                        onClick={() => onToggleDirection('morning')}
                        className={`flex-1 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all duration-150 ${(!route.direction || route.direction === 'morning') ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-700'}`}
                      >
                        Morning (Coming)
                      </button>
                      <button
                        onClick={() => onToggleDirection('evening')}
                        className={`flex-1 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all duration-150 ${(route.direction === 'evening') ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-700'}`}
                      >
                        Evening (Going)
                      </button>
                    </div>
